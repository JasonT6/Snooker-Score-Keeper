...
- `app/hooks/usePlayerRecognition.ts` loads the Human face-description model, creates enrollment descriptors from the selected MoveNet track’s enlarged head crop, holds each player’s descriptor gallery and thumbnail in React memory, releases stale mappings after a ten-second absence, processes a returning unassigned track through the identical head-crop pipeline, and explicitly binds its new MoveNet ID back to the matching stable player identity.
...
Camera streams, face descriptors, and face thumbnails are never persisted or uploaded. MoveNet and Human inference run in the browser through WebGL. Only player names, face-consent choices, and match length are stored in localStorage after setup; the two face memories disappear when the page closes. MoveNet pose IDs are temporary and cannot be forced to reuse an expired number. The app instead maintains a stable Player 1/Player 2 identity layer. Enrollment and re-identification both use the same enlarged tracked-head crop. After a player is absent for ten seconds the stale ID mapping is released; when that player returns and looks at the camera, the new track’s face is compared against both session galleries and the new ID is bound after two matching observations. Clothing is deliberately not used as identity because it is not a reliable person descriptor.
