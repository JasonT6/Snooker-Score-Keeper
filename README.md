...
- `app/hooks/usePlayerRecognition.ts` loads the Human face-description model, holds each face descriptor and thumbnail in React memory, carries known identities across the registration-to-framing video-element handoff, matches smaller faces in the wide table view, and rebinds new MoveNet IDs after a player returns.
...
Camera streams, face descriptors, and face thumbnails are never persisted or uploaded. MoveNet and Human inference run in the browser through WebGL. Only player names, face-consent choices, and match length are stored in localStorage after setup; the two face memories disappear when the page closes. MoveNet pose IDs are temporary. The app preserves a short-lived position anchor while switching from registration to table framing so a replacement track can immediately inherit the known player; face matching remains the authoritative recovery path after a longer absence and requires a minimum similarity, a clear lead over the other player, and two consecutive matches before rebinding. Clothing is deliberately not used as identity because it is not a reliable person descriptor.
