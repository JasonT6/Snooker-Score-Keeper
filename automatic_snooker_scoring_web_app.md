...
3. During setup, the app sees both players and creates a profile for each:
   ...
   - Capture at least five trained face descriptors per player locally only after consent is given. Keep the descriptor gallery and optional preview thumbnail in page memory only; do not write either to localStorage, IndexedDB, or a backend, and discard them when the page closes.
   ...
   - MoveNet owns its temporary numeric IDs, so maintain a separate stable Player 1/Player 2 identity layer. Detect every enrollment and returning face in the full camera frame and associate each face with the overlapping MoveNet track, using an enlarged tracked-head crop only as a face-search fallback for distant faces. Release a stale ID after a ten-second absence, compare every returning face against both session galleries, and bind the corresponding new MoveNet ID after repeated confident matches. Never bind by position alone or merely because exactly one person or unassigned track is visible. Do not guess when the face is not visible or cannot be associated with a body track.
...
- Use `@vladmandic/human` face descriptors for on-device player re-identification. During enrollment, capture at least five descriptors and associate the detected face/body overlap with the selected player's current track; reject any sample that cannot be linked to that track.
