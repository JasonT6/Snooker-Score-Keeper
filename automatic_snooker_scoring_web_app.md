...
3. During setup, the app sees both players and creates a profile for each:
   ...
   - Preserve identity through setup-screen video-element replacements by carrying a short-lived pose-position anchor into the new view. Re-identify a returning player from the face descriptor and rebind any new MoveNet pose ID after the player leaves the frame, is occluded long enough to lose the track, or the tracker restarts. Require a confident face match after that short handoff window and do not guess when the face is not visible.
