...
3. During setup, the app sees both players and creates a profile for each:
   ...
   - Preserve identity through setup-screen video-element replacements by carrying a short-lived pose-position anchor into the new view. MoveNet owns its temporary numeric IDs, so maintain a separate stable Player 1/Player 2 identity layer. Generate enrollment descriptors from the selected MoveNet track’s enlarged head crop. Release its stale ID after a ten-second absence, apply the identical crop to every newly unassigned returning track, compare the resulting face against both session-only descriptor galleries, and bind the new MoveNet ID after repeated confident matches. Do not guess when the face is not visible.
