...
3. During setup, the app sees both players and creates a profile for each:
   ...
   - Preserve identity through setup-screen video-element replacements by carrying a short-lived pose-position anchor into the new view. MoveNet owns its temporary numeric IDs, so maintain a separate stable Player 1/Player 2 identity layer. Detect enrollment and returning faces in the full camera frame and associate each face with the overlapping MoveNet track, using an enlarged tracked-head crop only as a fallback for distant faces. Release a stale ID after a ten-second absence, compare a returning face against both session galleries, and bind the new MoveNet ID after repeated confident matches. If a close face is recognized before its body is trackable, retain that identity match briefly and bind it only when exactly one new unassigned track appears. Do not guess when the face is not visible.
