...
3. During setup, the app sees both players and creates a profile for each:
   ...
   - Preserve identity through setup-screen video-element replacements by carrying a short-lived pose-position anchor into the new view. MoveNet owns its temporary numeric IDs, so maintain a separate stable Player 1/Player 2 identity layer. For every newly unassigned body track, crop and enlarge its head region, compare its face against the player’s session-only descriptor gallery, and bind the new MoveNet ID after repeated confident matches. Do not guess when the face is not visible.
